...
//   pair_index = 3*(c_even + 1) + (c_odd + 1), c in {-1, 0, +1} -> index in {0..8}.
...
// LUT-based binary dot product (SSSE3 + SSE4.1).
//
// Weight packing: each byte holds 4 pair-indices at 2 bits each (high pair first).
//   pair_index = 2*(w_even==+1) + (w_odd==+1), w ∈ {-1,+1} → index ∈ {0..3}.
//   n/8 weight bytes total for n elements (same density as mad_binary_dot).
//
// n must be a multiple of 16.
int32_t lut_binary_dot(const uint8_t* weights, const int8_t* activations, int n) {
    ...
    for (int i = 0; i < n / 16; i++) {
        ...
        // 4-entry LUT for all binary pair combinations:
        //   idx 0: (-1,-1) → -a_e - a_o
        //   idx 1: (-1,+1) → -a_e + a_o
        //   idx 2: (+1,-1) →  a_e - a_o
        //   idx 3: (+1,+1) →  a_e + a_o
        ...
        // Unpack 2 weight bytes into 8 pair-indices (2 bits each, high pair first).
        const uint8_t* wb = weights + i * 2;
        __m128i idx = _mm_set_epi16(
            wb[1] & 0x3, (wb[1] >> 2) & 0x3,  // pairs 7, 6
            (wb[1] >> 4) & 0x3, (wb[1] >> 6) & 0x3,  // pairs 5, 4
            wb[0] & 0x3, (wb[0] >> 2) & 0x3,  // pairs 3, 2
            (wb[0] >> 4) & 0x3, (wb[0] >> 6) & 0x3   // pairs 1, 0
        );
